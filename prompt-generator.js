...
const tissueBoxContainmentRule =
  "【纸巾盒图案边界硬约束与强制裁切】所有图案必须以图2纸巾盒当前可见表面为最终画布。图案可以比纸巾盒可见范围大，但最终成图只能保留纸巾盒表面边界以内的部分，溢出上、下、左、右边缘的部分必须直接裁掉、删除，不能出现在盒体外、不能被缩小后保留、不能悬空、不能形成盒外碎片或重复图案。马的耳朵、头部、鬃毛、四肢、尾巴，以及月亮、花叶、星星、云、装饰和所有玻璃拼片，只要超过纸巾盒可见边界，就沿着盒体边缘直接裁断，盒外部分完全消失。若可见面是3:1或类似横向比例，最终图案必须严格适配这个边界，盒体边缘就是硬裁切线。任何盒外残留、越界图案、悬浮部件或边界外影子都判定为失败；宁可裁掉主体局部，也不能超出纸巾盒范围。";
