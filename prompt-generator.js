...
const materialReplacementPromptLibrary = [
  {
    name: "图案替换款图片映射总控",
    prompt:
      "图1是唯一图案与图像参考，只提取图1中的主体造型、图案、轮廓、象征元素和构图关系，删除图1自己的背景、边框、无关道具和文字。图2是唯一目标物体，通常是纸巾盒，必须严格保留图2的外形、结构、比例、尺寸、开口、盒体轮廓、边缘、转角、接缝、材质、原有颜色基底、背景、机位、光线、阴影、桌面和其他物体。图3是可选颜色参考图，只提取图3主体的颜色搭配、色相关系、饱和度、明暗层次和主辅色比例，不复制图3的造型、图案、构图、背景或物体。只更换图2目标物体表面的装饰图案，其他内容全部保持不变。",
  },
  {
    name: "只替换图2表面图案",
    prompt:
      "将图1图案重新适配到图2目标物体可见的正面、顶面或侧面区域。图案必须贴合目标物体原有的表面边界、转角、开口和结构，不能改变目标物体的几何形状，不能覆盖或破坏原本的功能结构。不要把图1整张图片直接贴上去，不要复制图1背景，不要把目标物体替换成另一种材质或产品。",
  },
  {
    name: "图案适配与识别锚点",
    prompt:
      "图1中的主体可以作为完整图案，也可以根据图2目标物体表面结构做连续延展、裁切或分区，但必须保留图1主体至少两个可辨识特征，不能改变主体类别。新增图案必须完全服从图2表面形状，不能伸出物体边界，不能增加第二主体、额外边框、挂链、底座或外框。",
  },
  {
    name: "颜色优先级",
    prompt:
      "颜色优先级固定如下：第一，如果本轮在工作台选择了色彩变量，以工作台选择的色彩搭配为主，图1和图3颜色不得覆盖所选色彩变量，只允许黑色、深灰、白色、乳白或象牙白作为轮廓和高光辅助色。第二，如果本轮没有选择工作台色彩变量且提供了图3，则以图3主体的颜色作为图案主要配色参考。第三，如果本轮没有选择工作台色彩变量且没有图3，则使用图1图案原本的色相体系和颜色分布。",
  },
  {
    name: "保持不变与负面要求",
    prompt:
      "画面必须是同一个目标物体、同一机位、同一背景、同一灯光、同一构图，只允许目标物体表面图案发生变化。保持目标物体原来的阴影、桌面和倒影不变，不把阴影染色。不要增加第二个物体，不要改变背景，不要改变桌面，不要改变原有材质，不要增加文字、水印、标志、挂链、底座、透明边框或其他装饰。",
  },
];
...
优先选择“图2目标物体的外形、结构、背景、机位、光线和其他内容完全不变，只更换了表面图案”的效果最准确的一张。

图片映射验收：图1是唯一图案参考，图2是唯一目标物体，图3是可选颜色参考。图1的图案必须已经适配到图2表面，图2必须仍然是同一个物体，不能变成另一种产品。

颜色验收：如果工作台选择了色彩变量，最终图案必须以工作台色彩为主；如果未选择颜色且提供了图3，必须以图3主体颜色为主要配色；如果未选择颜色且没有图3，则保持图1原本颜色。

保持不变验收：目标物体以外的背景、桌面、物件、机位、构图、灯光、阴影和倒影必须与原图一致。目标物体的外形、结构、比例、开口、接缝、材质和原有颜色基底不得变化。

严禁选择出现以下情况的图片：改变目标物体形状，复制图1背景，复制图3造型，增加挂链、底座、外框、透明边框、第二主体、文字或水印。

选定后，后续所有生成必须以这张照片为唯一锚点，严格冻结目标物体外形、结构、比例、背景、机位、光线、阴影和图案位置，只允许图案内部颜色按本轮优先级规则调整。`;

const materialReplacementProductRule =
  "【产品款式最高优先级：图案替换款】图1是唯一图案与图像参考，图2是唯一目标物体，图3是可选颜色参考。必须完整保留图2目标物体的外形、结构、比例、尺寸、开口、盒体轮廓、边缘、转角、接缝、材质、原有颜色基底、背景、机位、光线、阴影、桌面和其他物体，只更换图2表面的装饰图案。图1负责提供图案主体、轮廓、构图和象征元素，不提供背景和无关道具。图3只负责提供主体颜色搭配、色相关系、饱和度、明暗层次和主辅色比例，不提供造型、图案、构图或背景。工作台色彩变量优先级最高；没有工作台选色时优先使用图3主体颜色；没有工作台选色也没有图3时使用图1原本颜色。不得改变目标物体类别和几何结构，不得增加挂链、吊环、书挡、木底座、托盘、支架、外框、透明背板或额外玻璃结构。";

const materialReplacementSubjectLockRule =
  "以图2目标物体为唯一主体，严格保持它的物体类别、外形、轮廓、比例、结构、开口、视角、裁切、遮挡关系、识别特征、材质、原有颜色基底、原有五金和原始配件不变。只允许替换目标物体表面的装饰图案，不允许改变物体几何结构，不允许把它重新设计成另一件商品。";

const materialReplacementThreeImageRule =
  "请生成本场景的3张独立图片，不要拼图。三张必须保持同一目标物体、同一外形结构、同一视角范围、同一背景、同一机位、同一灯光、同一阴影和同一构图，只允许物体表面图案及其颜色变化。三张都必须表现为同一个目标物体完成图案替换，不能改变物体类别、轮廓、比例、姿势或可见部件。";

const materialReplacementPlacementRule =
  "【图案替换款固定展示方式】目标物体必须沿用原图的位置、支撑方式、背景、桌面、空间关系、镜头距离、机位、视角、光线、阴影和倒影，不强制改成悬挂、书挡或摆件，也不能为了场景需要增加挂链、吊环、支撑杆、透明背板、整圈边框或其他不存在的结构。只允许目标物体表面的图案发生变化。";

const materialReplacementNegativeRule =
  "不要改变目标物体的类别、外形、轮廓、比例、结构、视角、裁切、遮挡关系、开口、接缝、材质、原有颜色基底和功能部件；不要改变背景、桌面、机位、灯光、阴影和倒影；不要增加挂链、吊绳、吊环、挂钩、书挡、木底座、托盘、支架、外框、透明背板或整圈玻璃边框；不要新增第二主体；不要复制图1的背景、图3的造型和构图；不要文字、水印、拼图或复杂背景。";

const materialReplacementColorRule =
  "【颜色优先级】第一，如果本轮在工作台选择了色彩变量，以工作台选择的色彩搭配为主，图1和图3的颜色不得覆盖所选色彩变量，只允许黑色、深灰、白色、乳白或象牙白作为轮廓和高光辅助色。第二，如果本轮没有选择工作台色彩变量且提供了图3，则以图3主体的颜色作为图案主要配色参考。第三，如果本轮没有选择工作台色彩变量且没有图3，则使用图1图案原本的色相体系和颜色分布。颜色只作用于目标物体表面的图案，不改变目标物体原有基底、背景、桌面、阴影或倒影颜色。";
...
function buildPrompt(analysis, background, angle, timeOfDay, shootingStyle, index) {
  ...
  return [
    ...
    `时间氛围：${timeOfDay.text}`,
    "",
    `${backgroundBudgetRule}${squareImageRatioRule}${isMaterialReplacementProduct() ? "" : singleLayerOcclusionRule}`,
    "",
    themeIsolationRule,
    "",
    vividGlassColorRule,
...
function renderSubjectLibrary() {
  ...
  if (isMaterialReplacementProduct()) {
    ...
    elements.shapeVariableDescription.textContent =
      "图案替换款锁定图2目标物体的外形、背景和结构，只用图1替换表面图案，图3仅作为可选颜色参考。";
...
function renderMaterialLibrary() {
  ...
  elements.materialPromptBox.value = formatPromptGroup(
    ...
    materialItems.map((item) => ({
      ...item,
      prompt: `${isMaterialReplacementProduct() ? "" : flatGlassRule}${colorStyleRule}${isMaterialReplacementProduct() ? "" : coloredShadowRule}${materialBaseRule}${stripSinglePromptCounts(item.prompt)}${getProductMaterialPriorityRule()}${buildEtsyShopStylePriorityRule(colorVariable)}`,
    })),
    [
      "本轮总共只生成3张照片。以下所有要求一次应用于这3张照片，不要按照每一条提示词分别再生成3张。",
      squareImageRatioRule,
...
${isMaterialReplacementProduct() ? "" : singleLayerOcclusionRule}
...
function renderProductUploadMode() {
  ...
  if (!isBookend) {
    elements.productTypeHint.textContent = isMaterialReplacement
      ? "图案替换款：工作台上传图1图案；到豆包按“图1图案、图2目标物体、图3可选颜色”放图，只更换目标物体表面图案。"
      : "挂链款保持现有流程，上传一张主体图即可。";
